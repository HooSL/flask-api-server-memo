...
class MemoResource(Resource):
    ...
    def post(self):
        ...
        #1-1. JWT 인증토큰에서는 user_id값을 가져온다.
        ...
        return {'result' : '저장완료.'} , HTTPStatus.OK
...
class MemoInfoResource(Resource):
    ...
    @jwt_required()
    def delete(self,memo_id): #delete는 body가 없다

        user_id = jwt_required()
        #1. 먼저 memo_id로 해당 메모를 가져와서 해당 메모의 user_id가 JWT의 토큰에서 뽑은
        #   user_id가 같은지를 먼저 확인한다.
        ...
        try :
            ...
            if connection.is_connected():
                ...
                connection.close()
                print('MySQL connection is closed')
        return {'result' : '삭제가 정상적으로 동작했습니다.'}, HTTPStatus.OK
